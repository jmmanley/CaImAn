...
# 9: sue_ann_k37
# 10: Jan-AMG_exp3_001
SNR_thr = np.linspace(0.5, 2.4, 20)
CNN_thr = np.linspace(0.3, 0.75, 10)
...
global_params = {'min_SNR': 0.8,  # minimum SNR when considering adding a new neuron
                 ...
                 'min_num_trial': 9,  # minimum number of times to attempt to add a component
                 'use_peak_max': True,
                 'thresh_CNN_noisy': .65,
                 'sniper_mode': True
                 }
...
if preprocessing_from_scratch:
    ...
    for ind_dataset in [0]: #range(9):
        ...
        #min_SNR = SNR_thr[ID[0]]
        min_SNR = 1 #global_params['min_SNR']
